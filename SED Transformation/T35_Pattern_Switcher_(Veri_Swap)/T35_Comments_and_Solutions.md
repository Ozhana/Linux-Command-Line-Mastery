...
| \( and the \) when separating groups with fixed characters (space, hyphen, colon) of the group outside be careful to let go.<br><br>Sample template: s/\[(.*)\] - (.*): (.*) \((.*)\)/\2 \| \3 \| \4 \| [\1]/ \| \( ve \) ile grupları ayırırken aradaki sabit karakterleri (boşluk, tire, iki nokta) grubun dışında bırakmaya dikkat edin.<br><br>Örnek şablon: s/\[(.*)\] - (.*): (.*) \((.*)\)/\2 | \3 | \4 | [\1]/ |
