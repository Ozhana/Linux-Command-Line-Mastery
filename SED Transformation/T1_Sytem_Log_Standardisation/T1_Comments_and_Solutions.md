...
| HINTS | IPUCLARI |
| A. sed -e 's/A/B/g' -e 's/C/D/g' dosyaits structure makes multiple changes at once. <br><br> B. s/aranan/yeni/g at the end g (global) changes all matches in the row. If two in a row 127.0.0.1 if g if you don't use it, only the first one changes. | A. sed -e 's/A/B/g' -e 's/C/D/g' dosya yapısı birden fazla değişikliği tek seferde yapar. <br><br> B. s/aranan/yeni/g sonundaki g (global), satırdaki tüm eşleşmeleri değiştirir. Eğer bir satırda iki tane 127.0.0.1 varsa g kullanmazsanız sadece ilki değişir |
